...
const PAYMENT_URL = 'https://vida.bauklar.com/purchase/report?source=api_error';
...
const TIER_QUOTAS: Record<ApiTier, number> = {
  free: 10,        // IP-based limit (unauthenticated)
...
/**
 * Generate Official ViDA Compliance PDF
 */
function generateComplianceReportPDF(options: {
  ...
  const status = result.valid && score >= 80 ? 'ready' :
  ...
  doc.setFontSize(9);
  doc.text(`Timestamp: ${timestamp}`, 35, 200);
...
router.get('/api/quota', async (request: Request, env: Env) => {
  try {
    ...
    // Differentiate between Unauthenticated (Free) and Authenticated (Paid)
    if (auth.tier === 'free') {
      return createResponse({
    ...
    return createResponse({
      mode: 'authenticated',
      tier: auth.tier,
      limit: `${TIER_QUOTAS[auth.tier || 'starter']}/month`,
      remaining: auth.remaining ?? 0,
      valid: auth.valid,
    });
...
router.post('/api/report', async (request: Request, env: Env) => {
  try {
    ...
    const filename = `Official-ViDA-Compliance-Audit-${Date.now()}.pdf`;
